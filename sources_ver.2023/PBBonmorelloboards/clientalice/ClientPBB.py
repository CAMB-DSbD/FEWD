...
date            : 9 Sep 2023, Computer Lab, University of Cambridge
version         : __ 
usage           : 
notes           :
compile and run : 
                : On a remote host (ex morello-camb-2.sm.cl.cam.ac.uk) run
                : % python3 serPBB.py 
                : On another computer, for example morello-camb-1.sm.cl.cam.ac.uk 
                : cm770@morello-camb-1: python ClientPBB.py 
...
class ClientPBB():
 ...
 def __init__(self, clientname= "cliAlice", server=SERVER_NAME, port=LOCAL_PORT):
 ...
 #def sockconnect(self, ser=SERVER_NAME, port=LOCAL_PORT):
     #self.server= ser
     #self.port= port
